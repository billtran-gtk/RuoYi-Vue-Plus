package org.dromara.huahao.domain.vo;

import cn.idev.excel.annotation.ExcelIgnore;
import cn.idev.excel.annotation.ExcelProperty;
import cn.idev.excel.annotation.write.style.ColumnWidth;
import com.fasterxml.jackson.annotation.JsonFormat;
import io.github.linpeilie.annotations.AutoMapper;
import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.NoArgsConstructor;
import org.dromara.common.excel.annotation.ExcelDictFormat;
import org.dromara.common.excel.annotation.ExcelRequired;
import org.dromara.common.excel.convert.ExcelDictConvert;
import org.dromara.huahao.domain.HAcceptanceForm;

import java.util.Date;
import java.util.Map;

@Data
@AllArgsConstructor
@NoArgsConstructor
@AutoMapper(target = HAcceptanceForm.class)
public class HAcceptanceFormVo {

    /**
     * 主键
     */
    @ExcelIgnore
    private Long id;

    /**
     * 租户编号
     */
    @ExcelIgnore
    private String tenantId;

    /**
     * 验收单号
     */
    @ExcelRequired
    @ExcelProperty(value = "Acceptance No")
    private String accNo;

    /**
     * 文件名
     */
    @ExcelRequired
    @ExcelProperty(value = "File Name")
    private String fileName;

    /**
     * 状态（0：未开始，1：收料中，2：已完成）
     */
    @ExcelRequired
    @ExcelProperty(value = "Status", converter = ExcelDictConvert.class)
    @ExcelDictFormat(dictType = "acceptance_form_status")
    private String status;

    /**
     * 创建者
     */
    @ExcelIgnore
    private Long createBy;

    /**
     * 创建时间
     */
    @JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss")
    @ExcelRequired
    @ExcelProperty(value = "Create Time")
    @ColumnWidth(20)
    private Date createTime;

    /**
     * 更新者
     */
    @ExcelIgnore
    private Long updateBy;

    /**
     * 更新时间
     */
    @JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss")
    @ExcelIgnore
    private Date updateTime;

    /**
     * 创建部门
     */
    @ExcelIgnore
    private Long createDept;

    /**
     * 删除标志（0代表存在 1代表删除）
     */
    @ExcelIgnore
    private String delFlag;

    /**
     * 请求参数（用于接收日期范围等查询条件）
     */
    @ExcelIgnore
    private Map<String, Object> params;
}
